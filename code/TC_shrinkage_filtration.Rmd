...
FOA_TC_normal_res_table <- lfcShrink(dds12_filt,
                              coef=2,
                              type="normal")

FOA_TC_apeglm_res_table <- lfcShrink(dds12_filt,
                              coef=2,
                              type="apeglm") # lfcThreshold = 0.585) 

FOA_TC_ashr_res_table   <- lfcShrink(dds12_filt,
                              coef=2,
                              type="ashr")
...
gene_counts_FOA_TC <- as.data.frame(c(nrow(FOA_TC_countmatrix),nrow(dds12_filt),(nrow(FOA_TC_countmatrix)-nrow(dds12_filt)),nrow(FOA_TC_normal_res_table),nrow(FOA_TC_apeglm_res_table),nrow(FOA_TC_ashr_res_table),nrow(FOA_TC_siggenes),nrow(FOA_TC_sig_genes_ashr),nrow(FOA_TC_sig_genes_normal)))
...
```{r}
#Run shrinkage estimates
GOA_TC_normal_res_table <- lfcShrink(dds15_filt,
                              coef=2,
                              type="normal")

GOA_TC_apeglm_res_table <- lfcShrink(dds15_filt,
                              coef=2,
                              type="apeglm") # lfcThreshold = 0.585) 

GOA_TC_ashr_res_table   <- lfcShrink(dds15_filt,
                              coef=2,
                              type="ashr")
...
GOA_TC_sig_genes_normal<- GOA_TC_normal_df %>%
  filter(padj < padj.cutoff)
write.table(GOA_TC_sig_genes_apeglm, file = "output/DEG_lists/Gill/siggene_OA_G.csv", row.names = FALSE)

#saving gene counts as a result of each filtration step
gene_counts_GOA_TC <- as.data.frame(c(nrow(GOA_TC_countmatrix),nrow(dds15_filt),(nrow(GOA_TC_countmatrix)-nrow(dds15_filt)),nrow(GOA_TC_normal_res_table),nrow(GOA_TC_apeglm_res_table),nrow(GOA_TC_ashr_res_table),nrow(GOA_TC_sig_genes_apeglm), nrow(GOA_TC_sig_genes_ashr),nrow(GOA_TC_sig_genes_normal)))
...
```{r}
#Run shrinkage estimates
GOW_TC_normal_res_table <- lfcShrink(dds16_filt,
                              coef=2,
                              type="normal")

GOW_TC_apeglm_res_table <- lfcShrink(dds16_filt,
                              coef=2,
                              type="apeglm") # lfcThreshold = 0.585) 

GOW_TC_ashr_res_table   <- lfcShrink(dds16_filt,
                              coef=2,
                              type="ashr")
...
GOW_TC_sig_genes_normal<- GOW_TC_normal_df %>%
  filter(padj < padj.cutoff)
write.table(GOW_TC_siggenes_apeglm, file = "output/DEG_lists/Foot/GOW_TC_siggene_apeglm.csv", row.names = FALSE)

#saving gene counts as a result of each filtration step
gene_counts_GOW_TC <- as.data.frame(c(nrow(GOW_TC_countmatrix),nrow(dds16_filt),(nrow(GOW_TC_countmatrix)-nrow(dds16_filt)),nrow(GOW_TC_normal_res_table),nrow(GOW_TC_apeglm_res_table),nrow(GOW_TC_ashr_res_table),nrow(GOW_TC_siggenes_apeglm),nrow(GOW_TC_sig_genes_ashr),nrow(GOW_TC_sig_genes_normal)))
...
```{r}
#Run shrinkage estimates
GDO_TC_normal_res_table <- lfcShrink(dds17_filt,
                              coef=2,
                              type="normal")

GDO_TC_apeglm_res_table <- lfcShrink(dds17_filt,
                              coef=2,
                              type="apeglm") # lfcThreshold = 0.585) 

GDO_TC_ashr_res_table   <- lfcShrink(dds17_filt,
                              coef=2,
                              type="ashr")
...
#saving gene counts as a result of each filtration step
gene_counts_GDO_TC <- as.data.frame(c(nrow(countmatrix_GDO),nrow(dds17_filt),(nrow(countmatrix_GDO)-nrow(dds17_filt)),nrow(GDO_TC_normal_res_table),nrow(GDO_TC_apeglm_res_table),nrow(GDO_TC_ashr_res_table),nrow(GDO_TC_siggenes),nrow(GDO_TC_sig_genes_ashr),nrow(GDO_TC_sig_genes_normal)))
