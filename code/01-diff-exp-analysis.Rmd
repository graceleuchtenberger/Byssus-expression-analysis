...
# Make file for sample and treatment ID (to be plugged into DESeq)
```{r}
# Get the tissue ID from the sample name
treatmentinfo$tissue <- substr(treatmentinfo$sample.1, nchar(treatmentinfo$sample.1) - 0, nchar(treatmentinfo$sample.1))

#Make subset of sample ID and tissue harvested
tissuesub <- treatmentinfo[, c("sample.1","tissue")]

#Specify FX samples for removal later
FXsamples <- tissuesub %>% 
  filter(tissuesub$tissue == "X") %>% 
  select(sample.1)
...
tissuesub <- tissuesub[order(tissuesub),]
tissuesub <- tissuesub[complete.cases(tissuesub), ]
tissuesub <- tissuesub[-c(1:4),]
