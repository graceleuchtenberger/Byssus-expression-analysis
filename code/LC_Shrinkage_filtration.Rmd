...
```{r}
#Run shrinkage estimates
GOA_LC_normal_res_table <- lfcShrink(dds8_filt,
                              coef=2,
                              type="normal")

GOA_LC_apeglm_res_table <- lfcShrink(dds8_filt,
                              coef=2,
                              type="apeglm") # lfcThreshold = 0.585) 

GOA_LC_ashr_res_table   <- lfcShrink(dds8_filt,
                              coef=2,
                              type="ashr")
...
GOA_LC_sig_genes_normal<- GOA_LC_normal_df %>%
  filter(padj < padj.cutoff)
write.table(GOA_LC_sig_genes, file = "output/DEG_lists/Gill/siggene_OA_G.csv", row.names = FALSE)

#saving gene counts as a result of each filtration step
gene_counts_GOA_LC <- as.data.frame(c(nrow(GOA_LC_countmatrix),nrow(dds8_filt),(nrow(GOA_LC_countmatrix)-nrow(dds8_filt)),nrow(GOA_LC_normal_res_table),nrow(GOA_LC_apeglm_res_table),nrow(GOA_LC_ashr_res_table),nrow(GOA_LC_sig_genes),nrow(GOA_LC_sig_genes_ashr),nrow(GOA_LC_sig_genes_normal)))
...
```{r}
#Run shrinkage estimates
GOW_LC_normal_res_table <- lfcShrink(dds9_filt,
                              coef=2,
                              type="normal")

GOW_LC_apeglm_res_table <- lfcShrink(dds9_filt,
                              coef=2,
                              type="apeglm") # lfcThreshold = 0.585) 

GOW_LC_ashr_res_table   <- lfcShrink(dds9_filt,
                              coef=2,
                              type="ashr")
...
#saving gene counts as a result of each filtration step
gene_counts_GOW_LC <- as.data.frame(c(nrow(GOW_LC_countmatrix),nrow(dds9_filt),(nrow(GOW_LC_countmatrix)-nrow(dds9_filt)),nrow(GOW_LC_normal_res_table),nrow(GOW_LC_apeglm_res_table),nrow(GOW_LC_ashr_res_table),nrow(GOW_LC_siggenes),nrow(GOW_LC_sig_genes_ashr),nrow(GOW_LC_sig_genes_normal)))
...
#saving gene counts as a result of each filtration step
gene_counts_GDO_LC <- as.data.frame(c(nrow(GDO_LC_countmatrix),nrow(dds10_filt),(nrow(GDO_LC_countmatrix)-nrow(dds10_filt)),nrow(GDO_LC_normal_res_table),nrow(GDO_LC_apeglm_res_table),nrow(GDO_LC_ashr_res_table),nrow(GDO_LC_siggenes),nrow(GDO_LC_sig_genes_ashr),nrow(GDO_LC_sig_genes_normal)))
...
```{r}
#Run shrinkage estimates
GTC_LC_normal_res_table <- lfcShrink(dds11_filt,
                              coef=2,
                              type="normal")

GTC_LC_apeglm_res_table <- lfcShrink(dds11_filt,
                              coef=2,
                              type="apeglm") # lfcThreshold = 0.585) 

GTC_LC_ashr_res_table   <- lfcShrink(dds11_filt,
                              coef=2,
                              type="ashr")
...
#saving gene counts as a result of each filtration step
gene_counts_GTC_LC <- as.data.frame(c(nrow(GTC_LC_countmatrix),nrow(dds11_filt),(nrow(GTC_LC_countmatrix)-nrow(dds11_filt)),nrow(GTC_LC_normal_res_table),nrow(GTC_LC_apeglm_res_table),nrow(GTC_LC_ashr_res_table),nrow(GTC_LC_siggenes),nrow(GTC_LC_sig_genes_ashr),nrow(GTC_LC_sig_genes_normal)))
